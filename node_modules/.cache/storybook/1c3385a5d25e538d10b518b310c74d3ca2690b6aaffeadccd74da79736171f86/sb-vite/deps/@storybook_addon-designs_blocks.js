...
// node_modules/@storybook/addon-designs/dist/blocks.mjs
...
the element.`);
  ...
  const createElement2 = React2.createElement;
  ...
  class ReactComponent extends Component {
    ...
    render() {
      ...
      for (const [k2, v] of Object.entries(userProps)) {
        if (reservedReactProperties.has(k2)) {
          props[k2 === "className" ? "class" : k2] = v;
          continue;
        }
        if (eventProps.has(k2) || k2 in element.prototype) {
          this._elementProps[k2] = v;
          continue;
        }
        props[k2] = v;
      }
      if (NODE_MODE5 && createElement2.name === "litPatchedCreateElement" && Object.keys(this._elementProps).length) {
        props["_$litProps$"] = this._elementProps;
        return createElement2(tag2, props);
      }
      if (!NODE_MODE5) {
  ...
  const ForwardedComponent = React2.forwardRef((props, __forwardedRef) => createElement2(ReactComponent, { ...props, __forwardedRef }, props === null || props === void 0 ? void 0 : props.children));
...
var E0 = (e, t) => {
  for (var a in t) u0(e, a, { get: t[a], enumerable: true });
};
var u;
var z0;
var x0;
var I0;
var H = k(() => {
  u = ({ config: e, defer: t = false }) => {
    let [a, l] = (0, import_react2.useState)(t ? void 0 : e.url), [r, n] = (0, import_react2.useState)(false);
    ...
    }, [t, e.url]), (0, import_react2.useEffect)(() => {
      n(false);
    }, [a]), vr("div", { css: z0 }, !r && vr(Zv, { css: x0 }, "Loading..."), vr("iframe", { css: I0, src: a, allowFullScreen: e.allowFullscreen, onLoad: () => n(true) }));
  }, z0 = Le`
  position: absolute;
  top: 0;
  right: 0;
...
function B0(e, t) {
  let a = new URL(e);
  a.hostname = a.hostname.replace(/^www\./, "embed."), a.searchParams.delete("embed_origin"), a.searchParams.set("embed-host", t);
  for (let [l, r] of a.searchParams) a.searchParams.delete(l), a.searchParams.set(l.replace(/_/g, "-"), r);
  return a.href;
}
var T;
var L0;
...
var V = k(() => {
  H();
  T = /https:\/\/[\w.-]+\.?figma.com\/([\w-]+)\/([0-9a-zA-Z]{22,128})(?:\/.*)?$/, L0 = (e) => T.test(e), A = ({ config: e }) => {
    let t = (0, import_react2.useMemo)(() => L0(e.url) ? { url: B0(e.url, e.embedHost || location.hostname), allowFullscreen: e.allowFullscreen } : (console.warn(`[storybook-addon-designs] The URL you specified is not valid Figma URL.
The addon fallbacks to normal iframe mode.For more detail, please check <https://www.figma.com/developers/embed>.`), e), [e.url, e.allowFullscreen, e.embedHost]);
    return vr(u, { defer: true, config: t });
  };
});
var X = {};
E0(X, { Figspec: () => S, default: () => y0 });
function P(e) {
...
function K(e) {
  return "absoluteBoundingBox" in e ? [e] : !e.children || e.children.length === 0 ? [] : e.children.map(K).reduce((t, a) => t.concat(a), []);
}
var Y;
var S;
var y0;
...
`;
  S = ({ config: e }) => {
    let [t, a] = (0, import_react2.useState)({ state: "loading" }), l = async (r) => {
      a({ state: "loading" });
      try {
        let n = e.url.match(T);
        if (!n) throw new Error(e.url + " is not a valid Figma URL.");
        let [, , o] = n, R = new URL(e.url).searchParams.get("node-id"), g = b0(e);
        if (!g) throw new Error("Personal Access Token is required.");
        let i = { "X-FIGMA-TOKEN": g }, w = new URL(`https://api.figma.com/v1/files/${o}`), c = new URL(`https://api.figma.com/v1/images/${o}`);
        if (c.searchParams.set("format", "svg"), !R) {
          let z = await fetch(w.href, { headers: i, signal: r }).then((C) => P(C)), p0 = K(z.document);
          c.searchParams.set("ids", p0.map((C) => C.id).join(","));
          let g0 = await fetch(c.href, { headers: i, signal: r }).then((C) => P(C));
          a({ state: "fetched", value: { type: "file", props: { documentNode: z, renderedImages: g0.images, link: e.url } } });
          return;
        }
        w.pathname += "/nodes", w.searchParams.set("ids", R), c.searchParams.set("ids", R);
        let [s, I] = await Promise.all([fetch(w.href, { headers: i, signal: r }).then((z) => P(z)), fetch(c.href, { headers: i, signal: r }).then((z) => P(z))]);
        a({ state: "fetched", value: { type: "frame", props: { nodes: s, renderedImage: Object.values(I.images)[0], link: e.url } } });
      } catch (n) {
        if (n instanceof DOMException && n.code === DOMException.ABORT_ERR) return;
        console.error(n), a({ state: "failed", error: n instanceof Error ? n.message : String(n) });
      }
    };
    switch ((0, import_react2.useEffect)(() => {
      ...
      return l(o.signal).then(n, n), () => {
        r || o.abort();
      };
    }, [e.url]), t.state) {
      case "loading":
        return vr(Zv, null, vr(import_react2.Fragment, null, "Loading Figma file..."));
      case "failed":
        return vr(Zv, null, vr(import_react2.Fragment, null, "Failed to load Figma file"), vr(import_react2.Fragment, null, t.error));
      case "fetched":
        return t.value.type === "file" ? vr(FigspecFileViewer2, { css: Y, ...t.value.props }) : vr(FigspecFrameViewer2, { css: Y, ...t.value.props });
    }
  }, y0 = S;
});
V();
...
H();
var Q = (e, t) => {
  let [a, l] = (0, import_react2.useState)([0, 0]), [r, n] = (0, import_react2.useState)(false), o = (0, import_react2.useCallback)((c) => {
    c.button === 0 && (l([c.screenX, c.screenY]), n(true));
  }, [n, l]), h = (0, import_react2.useCallback)((c) => {
    let s = c.touches[0];
    l([s.screenX, s.screenY]), n(true);
  }, [n, l]), R = (0, import_react2.useCallback)((c) => {
    r && l((s) => (e([c[0] - s[0], c[1] - s[1]]), c));
  }, [l, r, ...t]), g = (0, import_react2.useCallback)((c) => {
    let { screenX: s, screenY: I } = c;
    R([s, I]);
  }, [R]), i = (0, import_react2.useCallback)((c) => {
    let { screenX: s, screenY: I } = c.touches[0];
    R([s, I]);
  }, [l, r, ...t]), w = (0, import_react2.useCallback)(() => {
    l([0, 0]), n(false);
  }, [n, l]);
  return { onMouseDown: o, onMouseMove: g, onMouseUp: w, onMouseLeave: w, onTouchStart: h, onTouchMove: i, onTouchCancel: w, onTouchEnd: w };
};
var t0 = ({ children: e, className: t, style: a, defaultValue: l, value: r, onChange: n }) => {
  let [o, h] = (0, import_react2.useState)([0, 0]);
  (0, import_react2.useEffect)(() => {
    h(l || r || [0, 0]);
  }, [l]);
  let R = Q((i) => {
    n && n(i), h((w) => [w[0] + i[0], w[1] + i[1]]);
  }, [h, n]), g = (0, import_react2.useMemo)(() => {
    let i = r || o;
    return { transform: `translate(${i[0]}px, ${i[1]}px)` };
  }, [r, o]);
  return vr("div", { css: T0, className: t, style: a, ...R }, vr("div", { css: U0, style: g }, e));
};
var T0 = Le`
  position: relative;
  overflow: hidden;
...
var U0 = Le`
  ...
  left: 0;
`;
var a0 = d2.forwardRef(({ color: e = "currentColor", size: t = 14, ...a }, l) => d2.createElement("svg", { width: t, height: t, viewBox: "0 0 14 14", fill: "none", xmlns: "http://www.w3.org/2000/svg", ref: l, ...a }, d2.createElement("path", { d: "M6 3.5a.5.5 0 01.5.5v1.5H8a.5.5 0 010 1H6.5V8a.5.5 0 01-1 0V6.5H4a.5.5 0 010-1h1.5V4a.5.5 0 01.5-.5z", fill: e }), d2.createElement("path", { fillRule: "evenodd", clipRule: "evenodd", d: "M9.544 10.206a5.5 5.5 0 11.662-.662.5.5 0 01.148.102l3 3a.5.5 0 01-.708.708l-3-3a.5.5 0 01-.102-.148zM10.5 6a4.5 4.5 0 11-9 0 4.5 4.5 0 019 0z", fill: e })));
var l0 = d2.forwardRef(({ color: e = "currentColor", size: t = 14, ...a }, l) => d2.createElement("svg", { width: t, height: t, viewBox: "0 0 14 14", fill: "none", xmlns: "http://www.w3.org/2000/svg", ref: l, ...a }, d2.createElement("path", { d: "M4 5.5a.5.5 0 000 1h4a.5.5 0 000-1H4z", fill: e }), d2.createElement("path", { fillRule: "evenodd", clipRule: "evenodd", d: "M6 11.5c1.35 0 2.587-.487 3.544-1.294a.5.5 0 00.102.148l3 3a.5.5 0 00.708-.708l-3-3a.5.5 0 00-.148-.102A5.5 5.5 0 106 11.5zm0-1a4.5 4.5 0 100-9 4.5 4.5 0 000 9z", fill: e })));
var r0 = d2.forwardRef(({ color: e = "currentColor", size: t = 14, ...a }, l) => d2.createElement("svg", { width: t, height: t, viewBox: "0 0 14 14", fill: "none", xmlns: "http://www.w3.org/2000/svg", ref: l, ...a }, d2.createElement("path", { d: "M1.5 2.837V1.5a.5.5 0 00-1 0V4a.5.5 0 00.5.5h2.5a.5.5 0 000-1H2.258a4.5 4.5 0 11-.496 4.016.5.5 0 10-.942.337 5.502 5.502 0 008.724 2.353.5.5 0 00.102.148l3 3a.5.5 0 00.708-.708l-3-3a.5.5 0 00-.148-.102A5.5 5.5 0 101.5 2.837z", fill: e })));
var n0 = ({ onZoomIn: e, onZoomOut: t, onReset: a }) => vr(import_react2.Fragment, null, vr(xo, { onClick: e }, vr(a0, null)), vr(xo, { onClick: t }, vr(l0, null)), vr(xo, { onClick: a }, vr(r0, null)));
var o0 = (e, t) => {
  let [a, l] = (0, import_react2.useState)(1);
  (0, import_react2.useEffect)(() => {
    l(e);
  }, t);
  let r = (0, import_react2.useCallback)(() => {
    l((h) => h + 0.1);
  }, [l]), n = (0, import_react2.useCallback)(() => {
    l((h) => Math.max(h - 0.1, 0.1));
  }, [l]), o = (0, import_react2.useCallback)(() => {
    l(1);
  }, [l]);
  return { scale: a, zoomIn: r, zoomOut: n, resetZoom: o };
};
var F = ({ config: e }) => {
  let t = o0(e.scale || 1, [e.scale]), a = (0, import_react2.useMemo)(() => ({ transform: `scale(${t.scale})` }), [t.scale]);
  return vr("div", { css: Y0 }, vr(jo, { border: true }, vr("div", { style: { display: "grid", gridAutoFlow: "column", gap: "4px", alignItems: "center" } }, vr(import_react2.Fragment, { key: "left" }, vr("p", null, vr("b", null, "Image")), vr(bl, null), vr(n0, { onReset: t.resetZoom, onZoomIn: t.zoomIn, onZoomOut: t.zoomOut })))), vr(t0, { css: K0, defaultValue: e.offset }, vr("img", { css: X0, src: e.url, style: a })));
};
var Y0 = Le`
  position: absolute;
  top: 0;
...
V();
H();
var i0 = ({ config: e }) => vr("div", { css: j0 }, vr(Ai, { cancel: false, href: e.url, target: e.target ?? "_blank", rel: e.rel ?? "noopener", withArrow: e.showArrow ?? true }, e.label || e.url));
var j0 = Le`
  position: absolute;
  top: 0;
  right: 0;
...
H();
var a1 = (e) => {
  if (e.protocol !== "https:") return { valid: false, error: vr(import_react2.Fragment, null, "Expected HTTPS link, received ", vr("code", null, e.protocol), ".") };
  if (e.hostname !== "www.sketch.com") return { valid: false, error: vr(import_react2.Fragment, null, "Expected a hostname ", vr("code", null, "www.sketch.com"), ", received", " ", vr("code", null, e.hostname)) };
  let t = vr(import_react2.Fragment, null, "Expected pathname ", vr("code", null, "/s/<string>/a/<string>"), ", received", " ", vr("code", null, e.pathname), "."), a = e.pathname.split("/").slice(1);
  if (a.length < 4) return { valid: false, error: t };
  if (a[0] === "embed") return { valid: true, data: { url: e.href, offscreen: false } };
  let [l, r, n, o] = a;
  return l !== "s" || !r || n !== "a" || !o ? { valid: false, error: t } : { valid: true, data: { url: `https://www.sketch.com/embed/s/${r}/a/${o}`, offscreen: false } };
};
var h0 = ({ config: e }) => {
  let t = (0, import_react2.useMemo)(() => {
    let a = a1(new URL(e.url));
    return a.valid ? { ...a, data: { ...e, ...a.data } } : a;
  }, [e]);
  return t.valid ? vr(u, { defer: true, config: t.data }) : vr(Zv, null, vr(import_react2.Fragment, null, "Invalid Sketch URL"), vr(import_react2.Fragment, null, t.error));
};
var d0 = ({ tabs: e, deps: t = [] }) => {
  let [a, l] = (0, import_react2.useState)(e[0].id);
  return (0, import_react2.useEffect)(() => {
    l(e[0].id);
  }, t), vr(vl, { absolute: true, selected: a, actions: { onSelect: l } }, e.map((r) => vr("div", { key: r.id, id: r.id, title: r.name }, r.offscreen || a === r.id ? r.content : null)));
};
var i1 = (0, import_react2.lazy)(() => Promise.resolve().then(() => (O(), X)));
var m0 = ({ config: e }) => {
  if (!e || "length" in e && e.length === 0) return vr(Zv, null, vr(import_react2.Fragment, null, "No designs found"), vr(import_react2.Fragment, null, "Learn how to", " ", vr(Ai, { href: "https://github.com/storybookjs/addon-designs#3-add-it-to-story", target: "_blank", rel: "noopener", withArrow: true, cancel: false }, "display design preview for the story")));
  let t = [...e instanceof Array ? e : [e]].map((a) => {
    let l = { id: JSON.stringify(a), name: a.name || a.type?.toUpperCase() || "ERROR", offscreen: a.offscreen ?? true };
    switch (a.type) {
      case "iframe":
        return { ...l, content: vr(u, { config: a }) };
      case "figma":
        return { ...l, content: vr(A, { config: a }), offscreen: false };
      case "sketch":
        return { ...l, content: vr(h0, { config: a }) };
      case "figspec":
      case "experimental-figspec":
        return a.type === "experimental-figspec" && console.warn("[storybook-addon-designs] `experimental-figspec` is deprecated. We will remove it in v7.0. Please replace it to `figspec` type."), { ...l, content: vr(import_react2.Suspense, { fallback: "Preparing Figspec viewer..." }, vr(i1, { config: a })), offscreen: false };
      case "image":
        return { ...l, content: vr(F, { config: a }) };
      case "link":
        return { ...l, content: vr(i0, { config: a }) };
    }
    return { ...l, content: vr(Zv, null, vr(import_react2.Fragment, null, "Invalid config type"), vr(import_react2.Fragment, null, "Config type you set is not supported. Please choose one from", " ", vr(Ai, { href: "https://github.com/storybookjs/addon-designs/blob/master/packages/storybook-addon-designs/src/config.ts", target: "_blank", rel: "noopener", withArrow: true, cancel: false }, "available config types"))) };
  });
  return t.length === 1 ? vr("div", null, t[0].content) : vr(d0, { tabs: t, deps: [e] });
};
var s0 = "design";
var w1 = xr.div(({ theme: e }) => `
  font-family: ${e.typography.fonts.base};
  font-size: ${e.typography.size.s3}px;
  margin: 0;
`);
var m1 = xr.div(({ theme: e, height: t = "60%", collapsed: a }) => `
  position: relative;
  width: 100%;
  height: 0;
  padding: 0;
...
var R1 = xr(Zv)`
  ...
  transform: translate(-50%, -50%);
`;
var B = ({ children: e, collapsable: t = true, defaultCollapsed: a = false, placeholder: l, showLink: r = true, onCollapsedChange: n, ...o }) => {
  let [h, R] = (0, import_react2.useState)(!!a), g = r && "url" in o;
  return import_react2.default.createElement(w1, null, import_react2.default.createElement(m1, { collapsed: t && h, ...o }, t && h ? import_react2.default.createElement(R1, null, l) : e, import_react2.default.createElement(Ya, { actionItems: [t && { title: h ? "Show" : "Hide", onClick: () => {
    let i = !h;
    n && n(i, h), R(i);
  } }, g && { title: "Open in new tab", onClick: () => window.open(o.url, "_blank") }].filter((i) => !!i) })));
};
var oe = ({ placeholder: e, ...t }) => import_react2.default.createElement(B, { placeholder: e ?? "Design (Figma)", ...t }, import_react2.default.createElement(A, { config: { type: "figma", ...t } }));
var ce = ({ placeholder: e, ...t }) => import_react2.default.createElement(B, { placeholder: e ?? "Design (Figma-Spec)", ...t }, import_react2.default.createElement(S, { config: { type: "figspec", ...t } }));
var ie = ({ placeholder: e, ...t }) => import_react2.default.createElement(B, { placeholder: e ?? "Design (iframe)", ...t }, import_react2.default.createElement(u, { config: t }));
var he = ({ placeholder: e, ...t }) => import_react2.default.createElement(B, { placeholder: e ?? "Design (Image)", ...t }, import_react2.default.createElement(F, { config: { type: "image", ...t } }));
var s1 = xr.div`
  position: absolute;
  top: 0;
  right: 0;
...
var ve = (e) => {
  ...
  return import_react2.default.createElement(B, { placeholder: a ?? "Design", ...l }, import_react2.default.createElement(s1, null, import_react2.default.createElement(m0, { config: r.parameters[s0] })));
};
export {
  ve as Design,
  B as DocBlockBase,
